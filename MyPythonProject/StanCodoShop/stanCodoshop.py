...
def get_pixel_dist(pixel, red, green, blue):
    ...
    g_pixel = pixel.green
    b_pixel = pixel.blue
    dist = math.sqrt((red-r_pixel)**2+(green-g_pixel)**2+(blue-b_pixel)**2)
    return dist
...
def get_average(pixels):
    ...
    g_sum = 0
    b_sum = 0
    # Loop over the list to sum up the rgb total
    for pixel in pixels:
        r_sum += pixel.red
        g_sum += pixel.green
        b_sum += pixel.blue
...
def get_best_pixel(pixels):
    ...
    dist_min = 0
    for pixel in pixels:
        dist = get_pixel_dist(pixel, r_avg, g_avg, b_avg)
        # Compare each pixel
        if pixel == pixels[0]:        # The first comparison
            dist_min = dist
            best_pixel = pixel
        else:
            if dist < dist_min:
...
def solve(images):
    ...
    for y in range(height):
        for x in range(width):
            # Get a (x, y) pixel list
            for img in images:
                pixel = img.get_pixel(x, y)
                pixel_lst.append(pixel)
            # get the best pixel
            best_pixel = get_best_pixel(pixel_lst)
            # Put the best pixel back to the result image
